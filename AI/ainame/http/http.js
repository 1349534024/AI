      ...
      success: (res) => {
        ...
        if (res.statusCode === 200) {
          ...
          if (res.data && Array.isArray(res.data.detail)) {
            // 捕获 Pydantic 数据校验 422 错误 (取第一条报错)
          ...
          // 将错误抛出，供页面的 catch 处理（比如用来关掉 loading 动画）
          reject(res.data);
        }
      },
      fail: (err) => {
...
const uploadFile = (url, filePath) => {
  ...
  return new Promise((resolve, reject) => {
    uni.uploadFile({
      ...
      success: (res) => {
        if (res.statusCode === 200) {
          // 注意：uni.uploadFile 返回的 data 是字符串格式的 JSON，需要手动 parse
...
export default {
  ...
  getAdminUserStats: (params = {}) => {
    const query = Object.keys(params)
      ...
      .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(params[key])}`)
      .join('&');
    return request(`/admin/users/stats${query ? '?' + query : ''}`, { method: 'GET' });
  },
  getAdminUser: (userId) => request(`/admin/users/${userId}`, { method: 'GET' }),
  updateAdminUser: (userId, data) => request(`/admin/users/${userId}`, { method: 'PATCH', data }),
  freezeAdminUser: (userId) => request(`/admin/users/${userId}/freeze`, { method: 'POST' }),
  unfreezeAdminUser: (userId) => request(`/admin/users/${userId}/unfreeze`, { method: 'POST' }),
  resetAdminUserPassword: (userId, password) => request(`/admin/users/${userId}/reset-password`, { method: 'POST', data: { password } }),
